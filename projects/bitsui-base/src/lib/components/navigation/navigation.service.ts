...
export class FuseNavigationService {
   ...
   * @param navigation
   */
  register(key: string, navigation: FuseNavigationItem): void {
   ...
   * @returns {any | boolean}
   */
  getNavigationItem(id: string, navigation: FuseNavigationItem[] | null = null): any | boolean {
    if (!navigation) {
      navigation = this.getCurrentNavigation();
    }

    for (const item of navigation as FuseNavigationItem[]) {
      if (item.id === id) {
        return item;
      }
   ...
   * @param parent
   */
  getNavigationItemParent(id: string, navigation: FuseNavigationItem[] | null = null, parent: FuseNavigationItem[] | FuseNavigationItem | null = null): any {
    if (!navigation) {
      navigation = this.getCurrentNavigation();
      parent = navigation;
    }
   ...
   * @param id
   */
  addNavigationItem(item: FuseNavigationItem, id: string): void {
    // Get the current navigation
    const navigation: FuseNavigationItem[] = this.getCurrentNavigation();
   ...
   * @param properties
   */
  updateNavigationItem(id: string, properties: FuseNavigationItem): void {
